...
First, the cell tape is replaced by a memory with addressing for N locations of 1 bit, where each data location can contain either a zero or a one, as shown in Figure 35. In this new representation, one cell corresponds to one data locality while the tape head corresponds to the register known as the _data pointer_ (DP) register, which indicates the address of the data locality in turn. 
